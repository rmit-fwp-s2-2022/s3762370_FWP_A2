...
import { getUser, deleteUsers, delPostByUsers, findUser } from "../data/repository"
...
const Profile = (props) => {
    ...
    const deleteUser = async () => {
        setShowModal(false)
        const user = await findUser(props.userLoggedIn)
        await deleteUsers({
            user_id: user.user_id,
        })
        await delPostByUsers({
            user_id: user.user_id,
        })
        localStorage.clear()
        navigate("/login")
                ...
                <div>
                    ...
                    <div className='flex border-b border-slate-300'>
                        ...
                        <Link className='ml-10 my-auto' to={"/edituser"}>
                            <a target="_blank" className=" text-gray-400 hover:text-gray-500 active:text-gray-600 transition duration-100 cursor-pointer">
                        ...
                        <a target="_blank" className="ml-5 my-auto text-gray-400 hover:text-gray-500 active:text-gray-600 transition duration-100 cursor-pointer" onClick={() => setShowModal(true)}>
